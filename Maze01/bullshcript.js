(function () {
  ...
  async function somerandomStartCrap() {
	...
	const mazescene = BS.BanterScene.GetInstance();
	// Create the landing platform
	landingPlatform();
	// Wait for Unity to be ready
    const waitingForUnity = async () => { while (!mazescene.unityLoaded) { await new Promise(resolve => setTimeout(resolve, 500)); } };
    await waitingForUnity(); console.log("SCRIPT: Unity-Loaded");
    setTimeout(() => { loadSettings(mazescene); }, 1000);
  };

  function loadSettings(mazescene) {
